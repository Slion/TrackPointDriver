...
#pragma alloc_text (PAGE, MouFilter_EvtDriverContextCleanup)
#endif
...
--*/
{
	...
	attributes.EvtCleanupCallback = MouFilter_EvtDriverContextCleanup;
  ...
  --+*/
{
	...
	if (devExt->UpperIsrHook) {
		...
		if (!retVal || !(*ContinueProcessing)) {
			...
			FuncExit(TRACE_DRIVER, "retVal=%d", retVal);
			return retVal;
		}
	}
...
--*/
{
	...
	// Track state of our middle button
	static BOOLEAN middleButtonDown = FALSE;
	PDEVICE_EXTENSION   devExt;
	WDFDEVICE   hDevice;
	...
	for (PMOUSE_INPUT_DATA InputData = InputDataStart; InputData < InputDataEnd; ++InputData) {
		...
		// Check if middle button went up
		if (InputData->ButtonFlags & MOUSE_MIDDLE_BUTTON_DOWN) {
			TraceVerbose(
				TRACE_DRIVER,
				"KTT:MiddleButton: DOWN"
			);
			middleButtonDown = TRUE;
			// Filter out middle button
			InputData->ButtonFlags &= ~MOUSE_MIDDLE_BUTTON_DOWN;
		}

		// Check if middle button went up
		if (InputData->ButtonFlags & MOUSE_MIDDLE_BUTTON_UP) {
			TraceVerbose(
				TRACE_DRIVER,
				"KTT:MiddleButton: UP"
			);
			middleButtonDown = FALSE;
			// Filter out middle button actions
	...
	// I guess that's passing on our filtered data input
	// We could use that to do some injection maybe? Yes that's what it says in the function doc above :)
...
VOID
MouFilter_EvtDriverContextCleanup(
